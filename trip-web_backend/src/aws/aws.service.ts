...
export class AwsService {
  ...
  constructor(private configService: ConfigService) {
    ...
    this.s3Client = new S3Client({
      region: this.configService.get(process.env.AWS_REGION), // AWS Region
      credentials: {
        accessKeyId: this.configService.get(process.env.AWS_ACCESS_KEY_ID), // Access Key
  ...
  async imageUploadToS3(
    ...
    const command = new PutObjectCommand({
      Bucket: this.configService.get(process.env.AWS_S3_BUCKET_PROFILE), // S3 버킷 이름
